...
class LoginClass extends DBController {
	...
	function rsa_decrypt($str){ 
		// 구글링으로 직접 해결하시길... 개고생한 거라 오픈하기 싫음
	...
	function Sales_Date($date){
		...
		$weekday = date("w",strtotime($date)); // 일요일 0, 토요일 6
		switch($weekday){
			case 0: // 일요일
				$reg_date = date('Y-m-d', $_SERVER['REQUEST_TIME']-86400*2); 
				break;
			case 1: // 월요일
				$reg_date = date('Y-m-d', $_SERVER['REQUEST_TIME']-86400*3); 
				break;
			case 2: // 화요일
				$reg_date = date('Y-m-d', $_SERVER['REQUEST_TIME']-86400); 
				break;
			case 3: // 수요일
				$reg_date = date('Y-m-d', $_SERVER['REQUEST_TIME']-86400); 
				break;
